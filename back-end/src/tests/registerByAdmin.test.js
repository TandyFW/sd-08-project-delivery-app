...
const generateRole = () => Math.round(Math.random()) ? 'customer' : 'seller';
...
describe('POST /admin', () => {
  describe('quando NOVO usuário é cadastrado', () => {
    ...
    before(async () => {
      const { token } = await chai.request(server).post('/login').send(admin);
      response = await chai.request(server).post('/admin').set('Authorization', token ).send(clientData);
    });
    ...
    it('o objeto possui as propriedades "id", "name", "email", "password", "role"', () => {
  ...
  describe('quando NOVO registro está cadastrado no banco de dados', () => {
    ...
    before(async () => {
      response = await chai.request(server).post('/admin').send(clientData);
    });
